...
public class Utils {
    ...
    public static void getLocationUpdates(final Context context, final Intent intent, String broadcastevent)  {
        ...
        if (result != null) {
            ...
            final Location firstLocation = locations.get(0);
            ...
            String objectId = Preferences.getPrefs("objectId",context);
            ...
            System.out.println("MY REAL TIME objectID" + objectId);

            final ArrayList<String> myFriends = OneFragment.result;

//            for(int ai = 0; ai < myFriends.size(); ai++) {
//                String q = myFriends.get(ai);
//                System.out.println("aboutToFindInSharedPrefLOOPDOS " + q);

                ParseQuery<ParseObject> query = ParseQuery.getQuery("MyFirstClass");
// Retrieve the object by id
                query.getInBackground(objectId, new GetCallback<ParseObject>() {
                    public void done(ParseObject objectId, ParseException e) {
                        if (e == null) {
                            // Now let's update it with some new data. In this case, only cheatMode and score
                            // will get sent to the Parse Cloud. playerName hasn't changed.
                            objectId.put("latitude", firstLocation.getLatitude());
                            objectId.put("longitude", firstLocation.getLongitude());
                            System.out.println("en UTILS " + objectId.toString());
                            objectId.saveInBackground();
                        } else {
                            // Failed
                        }
                    }
